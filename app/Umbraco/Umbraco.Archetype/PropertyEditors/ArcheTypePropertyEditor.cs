				...
				foreach (var fieldset in archetype.Fieldsets)
				{
					foreach (var propDef in fieldset.Properties.Where(p => p.DataTypeGuid != null))
					{
						try
						{
							...
							var prop = new Property(propType, propDef.Value.ToString());
				...
				foreach (var fieldset in archetype.Fieldsets)
				{
					...
					foreach (var propDef in fieldset.Properties)
					{
						try
						{
							...
							propDef.Value = propEditor.ValueEditor.ConvertEditorToDb(propData, currentProperty != null ? currentProperty.Value.ToString() : null);
						}
						catch (Exception ex)
						{
							LogHelper.Error<ArchetypePropertyValueEditor>(ex.Message, ex);
